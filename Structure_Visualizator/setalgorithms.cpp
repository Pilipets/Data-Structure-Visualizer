...
StructureRepresentor *SetAlgorithms::getDifference(StructureRepresentor *s1, StructureRepresentor *s2)
{
    StructureRepresentor *s = MyFactory::getInstance()->createEssence(s1);
    this->saveDifference<int,int>(s,s1,s2);
    return s;
}
...
template<class K, class V>
void SetAlgorithms::saveDifference(StructureRepresentor *s, StructureRepresentor *s1, StructureRepresentor *s2)
{
    GetItemsVisitor<int,int> v1,v2;
    s1->accept(v1), s2->accept(v2);
    for(size_t i = 0; i < v1.amount; ++i){
        auto key_it2 = std::find(v2.keys.begin(), v2.keys.end(), v1.keys[i]);
        auto val_it2 = std::find(v2.values.begin(), v2.values.end(), v1.values[i]);
        if(val_it2 == v2.values.end() || key_it2 == v2.keys.end())
            s->insert(v1.keys[i],v2.values[i]);
    }
}
